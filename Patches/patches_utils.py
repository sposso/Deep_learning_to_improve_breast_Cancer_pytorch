...
def sample_patches(img,index,label, roi_mask,folder_s, folder_s10,list_s,list_s10,patch_size=224,
    ...
    if img.shape == roi_mask.shape:
        ...
        while sampled_abn < nb_abn:
            ...
            if overlap_patch_roi((x,y), patch_size, roi_mask, cutoff=pos_cutoff):
                ...
                list_s10.append({'path': path_s10, 'label': label})
        ...
        while sampled_bkg < start_sample_nb + nb_bkg:
            ...
            if not overlap_patch_roi((x,y), patch_size, roi_mask, cutoff=neg_cutoff):
                ...
                if sampled_bkg ==1:
                    ...
                    list_s.append({'path': bkg_s_path, 'label': 0})
                    

                else:

                    bkg_s10_path = os.path.join(folder_s10, f"bkg_{index}_{sampled_bkg}.png")
    
                    bkg= bkg.astype('uint16')
                    image_as_png(bkg,bkg_s10_path)
                    list_s10.append({'path': bkg_s10_path, 'label': 0})
